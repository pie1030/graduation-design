...
from model.blip_outputs import BlipOutput, BlipOutputFeatures
import numpy as np
...
class Blip2Qformer(Blip2Base):
        ...
        if freeze_vit:
            for name, param in self.visual_encoder.named_parameters():
                if "blocks.37" in name or "blocks.38" in name:
                    continue
                if 'ln_mid' in name or 'joint_positional_embedding' in name:
                    continue
                if 'bef_embedding' in name or 'aft_embedding' in name:
                    continue
                param.requires_grad = False
            # for name, param in self.visual_encoder.named_parameters():
            #     print(f"{name}: requires_grad={param.dtype}")
    ...
    def forward(self, samples):
        ...
        image = torch.cat([imageA, imageB], dim = 0) # [2b, 3, H, W]
        text = samples["text_input"]
        
        image_embeds = self.ln_vision(self.visual_encoder(image)) # [2b, ...]
        # image_embedsA, image_embedsB = torch.chunk(image_embeds, 2, dim = 0)
        ...
        """
        # loss_itc = (
        #     F.cross_entropy(sim_i2t, neg_mask, label_smoothing=0.1)
        #     + F.cross_entropy(sim_t2i, neg_mask, label_smoothing=0.1)
        # ) / 2
        #  F.cross_entropy(sim_i2t, targets, label_smoothing=0.1)
        ...
        whole_index = [i for i in range(bs)]
        image_embeds_neg = []
        for b in range(bs):
            connected_idx = torch.nonzero(neg_mask[b,:], as_tuple=True)[0].tolist()
            neg_indexes = torch.tensor(list(set(whole_index)-set(connected_idx)), dtype=float)
            neg_idx = torch.multinomial(neg_indexes, 1).item()
        ...
        attention_mask = torch.cat([query_atts, text_tokens.attention_mask], dim=1)   # [32, 64]
    ...
    def generate(
        ...
        imageA = samples["image_B"] # [b, 3, H, W]
        imageB = samples["image_A"] 
        image = torch.cat([imageA, imageB], dim = 0) # [2b, 3, H, W]  [64, 3, 224, 224]
        # text = samples["text_input"]
        ...
        # with self.maybe_autocast(dtype=torch.double):
        # image_embeds = torch.cat((image_embedsA, image_embedsB), dim=1)  # [32, 514, 1408]

        if not use_nucleus_sampling:
            image_embeds = image_embeds.repeat_interleave(num_beams, dim=0)  # [96, 514, 1408]
        else:
            num_beams = 1
        ...
        input_ids = (
            torch.LongTensor(imageA.size(0), 1)
            .fill_(self.tokenizer.bos_token_id)
            .to(image.device)
        )  # [32, 1]

        query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1)   # [96, 32, 768]
        # import pdb;pdb.set_trace();
        self.Qformer = self.Qformer.double()
        query_tokens = query_tokens.double()
        model_kwargs['encoder_hidden_states'] = image_embeds.double()
